...
def optimize(n, B, trade_off, p_i, o_i):
    ...
    Returns:
        res (OptimizeResult): Optimization result
    """

    # coeffs
    a = p_i * o_i - 1.0
    b = trade_off * o_i**2 * p_i * (1.0 - p_i)
    ...
    cons = {
        'type': 'eq', 'fun': lambda x: B - np.sum(x), 'jac': lambda x: -np.ones_like(x)
    }
...
def main():
    ...
    parser.add_argument(
        '--budget', type=float, default=100.0, help='Total budget for betting'
    )
    parser.add_argument(
    ...
    res = optimize(n_bets, budget, trade_off, win_probs, payout_odds)

    print("\nBet allocation:")
    for i, amount in enumerate(res.x):
        print(f"bet {i+1}: ${amount:.2f} (Win prob: {win_probs[i]:.2f}, Odds: {payout_odds[i]:.2f})")
    
    print(f"\nTotal allocated: ${np.sum(res.x):.2f}")
    print(f"Expected return: ${np.dot(win_probs * payout_odds - 1.0, res.x):.2f}")
